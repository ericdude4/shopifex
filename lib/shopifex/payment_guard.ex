defmodule Shopifex.PaymentGuard do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      @impl Shopifex.PaymentGuard
      def use_grant(_shop, %{remaining_usages: nil} = guard), do: guard

      def use_grant(shop, %{remaining_usages: remaining_usages} = guard) do
        guard
        |> Ecto.Changeset.change(%{remaining_usages: remaining_usages - 1})
        |> repo().update!()
      end
